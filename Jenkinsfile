...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="changegood/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/nftsqloriginal/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@13.125.143.134 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@13.125.143.134 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
        ...
        stage("Email Notification Test") {
            ...
            post {
                always {
                    ...
                    , recipientProviders: [buildUser()], subject: 'lee chang hee', to: 'nftsql@gmail.com')
                }
                success {
                    emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: 'lee chang hee', to: 'nftsql@gmail.com')
